...
def update_mult_dts_items(mult_dt_q:str, state_id:list[str],commodity: str, domain: str, data_item:str, init_vals: list[str])->Tuple[list[dict[str,str]], dict[str,str], dict[str,str]]:
    ...
    Takes in answer to previous question (mult_dt_q, a string), all previous selctions made according to the type of their values (dependent on what type of item they are),
...
def ask_barplot_xax(viz_type:str,state_id:list[str], commodity:str, domain:str, data_item:str, mult_dt_q:str, domain_category:Union[list[str],list], year:list[str], stat_type:str) ->Tuple[list[str], dict[str,str], dict[str,str]]:
    ...
    It then determines whether years have been selected by the user, and if they are valid by querying the irrigation database (if there aren't any valid domain categories or years, or 'One Data Item' has been chosen, the buttons don't display )
