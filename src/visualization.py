...
def form_x_tick_labels4(label_list:list[str], line_graph:bool=False, data_item:bool=False)->list[str]:
    '''
    Called by both 
    ...
    if yr_or_states == None: ##indicates that year or states aren't guaranteed to be x axis
        ...
        if 'domain_category' in params.keys(): #indicates only one data item specified, so data item is not on the x axis
            if len(params['domain_category']) >1:  ##indicates domain category is on the x axis
                ...
                x_tick_labels=form_x_tick_labels4(sorted(params['domain_category'])) #gets properly formatted x tick labels with line breaks to avoid overlap of text
            ...
            if len(params['data_item']) >1: ##indicates data item is the x axis
                ...
                x_tick_labels=form_x_tick_labels4(label_list=sorted(params['data_item']), line_graph=False, data_item=True) #gets properly formatted x tick labels with line breaks to avoid overlap of text
...
def make_line_graph(params:dict[str, list[str]], y_data:list[list[float]],operation:str, s_multiple_or_one:Union[str, None])->plotly.graph_objs._figure.Figure:
    ...
    if len(params['data_item'])>1: ##each line represents a data item
        ...
        data_list=form_x_tick_labels4(label_list=sorted(params['data_item']), line_graph=True, data_item=True) ##obtaining properly formatted labels for each line with line breaks to avoid large amount of  horizontal space taking up space on visualization
    elif 'domain_category' in params.keys():
        if len(params['domain_category'])>1: #each line represents a domain category

            data_list=form_x_tick_labels4(sorted(params['domain_category']), True)
